...
import { APP_ID, AZURE_FUNCTION_GET_KEYWORDS, AZURE_FUNCTION_UPDATE_PAGE } from '../constants/constants';
...
export const useAzureFunctions = (aadHttpClientFactory: AadHttpClientFactory) => {
    ...
    const getKeywordsUsingOpenAI = React.useCallback(
        async (content: string) => {
            return await callAzureFunction(AZURE_FUNCTION_GET_KEYWORDS, { content });
        },
        [callAzureFunction]
    );
    ...
    return { getKeywordsUsingOpenAI, updatePagePnPPowerShell };
};